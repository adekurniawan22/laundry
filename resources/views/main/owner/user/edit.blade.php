    ...
    <main class="page-content">
        ...
        <div class="row mx-0">
            <div class="card radius-10 w-100">
                <div class="card-body">
                    <form action="{{ route('owner.user.update', $user->id_user) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <select id="role" name="role" class="form-select @error('role') is-invalid @enderror">
                                ...
                                @foreach ($role as $roleItem)
                                    <option value="{{ $roleItem->id_role }}"
                                        {{ old('role', $user->id_role) == $roleItem->id_role ? 'selected' : '' }}>
                            ...
                            <select id="cabang" name="cabang" class="form-select @error('cabang') is-invalid @enderror">
                                ...
                                @foreach ($cabang as $cabangItem)
                                    <option value="{{ $cabangItem->id_cabang }}"
                                        {{ old('cabang', $user->id_cabang) == $cabangItem->id_cabang ? 'selected' : '' }}>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea id="alamat" name="alamat" class="form-control @error('alamat') is-invalid @enderror">{{ old('alamat', $user->alamat) }}</textarea>
                            @error('alamat')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
