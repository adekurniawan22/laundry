    ...
    <main class="page-content">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item" aria-current="page">
                            <span class="text-dark">Edit Cabang</span>
                        </li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="row ms-0 me-1">
            <div class="card radius-10 w-100">
                <div class="card-body">
                    <form action="{{ route('owner.cabang.update', $cabang->id_cabang) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea id="alamat" name="alamat" class="form-control @error('alamat') is-invalid @enderror">{{ old('alamat', $cabang->alamat) }}</textarea>
                            @error('alamat')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <select id="kontak" name="kontak" class="form-select @error('kontak') is-invalid @enderror">
                                ...
                                @foreach ($user as $item)
                                    <option value="{{ $item->id_user }}"
                                        {{ old('kontak', $cabang->kontak) == $item->id_user ? 'selected' : '' }}>
                                        {{ $item->nama }}</option>
                                @endforeach
                            </select>
                            @error('kontak')
